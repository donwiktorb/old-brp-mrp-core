                        ...
                        <div className="h-full w-full flex flex-col gap-4">
                            <div className="w-full h-1/4 flex justify-center items-center gap-4">
                                <div className="flex justify-center items-center flex-col w-full h-full p-2 gap-4">
                                    <textarea className="w-2/4 h-2/4 appearance-none p-2 border-black bg-pink-700 rounded-lg" value={msg} onChange={(e) => setMsg(e.target.value)}  >
                                    </textarea>
                                    <div className="w-2/4 gap-4 justify-center items-center grid grid-cols-7">
                                        <input type='text' value={money} className="bg-pink-700 p-2 rounded-lg text-center"></input>
                                        <input type='text' value={time} className="bg-pink-700 p-2 rounded-lg text-center"></input>
                                        <button className="bg-pink-700 p-2 rounded-lg" onClick={clearAll}>
                                            Wyczyść
                                        </button>
                                        <Link to="/ticket" state={{crimes: Crimes, money: money, time:time, crimeMsg: msg}}  className="bg-pink-700 p-2 rounded-lg">
                                            Ticket
                                        </Link>
                                        <button className="bg-pink-700 p-2 rounded-lg">
                                            Przedstaw
                                        </button>

                                    <div className='relative inline-block  '>
                                        <input onChange={sortIt} defaultValue="" className="bg-pink-700 focus:outline-none text-black rounded-lg p-2  w-full h-full" />
                            ...
                            <div className="w-full h-3/4 flex">
                                <div className="w-1/4 h-fit max-h-full flex flex-wrap gap-4 px-2 overflow-y-scroll pb-2">
                                    {Crimes.map((v, i) => {
                                        return <button className="bg-pink-700 border-red-500 border-2 grow p-2 w-fit h-fit rounded-lg" value={i} onClick={removeCrime}>
                                            {v.label} x{v.count}
                                        </button>
                                    })}
                                </div> 
                                        ...
                                        return <div className="w-full h-fit flex flex-col gap-4 ">
                                            ...
                                            <div className="w-full h-full grid grid-cols-4 gap-4 pr-4 overflow-y-scroll pb-2 ">
                                                {v.elements && v.elements.map((nv,ni) => {
                                                    return <button className="bg-pink-700 p-2 rounded-lg w-full h-fit" onClick={addCrime} value={`${i}-${ni}`}>
                                                        {nv.label}
                                                    </button>
                                                })}
                                            </div>
