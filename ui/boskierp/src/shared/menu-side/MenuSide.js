...
export default class MenuSide extends React.Component {
  constructor(props) {
    ...
    this.sides = {
      right:
        "w-full z-20 absolute h-full flex justify-end items-center text-white",
    ...
    this.main = {
      0: "overflow-y-scroll overflow-x-hidden flex flex-col gap-4 p-2",
    ...
    this.state = {
      ...
      elements: [
        ...
        // },
        {
          ...
          name: "Custom",
          checked: true,
          img: "https://boskierp.pl/img/ss/dwb_ORY3.png",
          elements: [
            {
              type: "radio",
  ...
  open(data) {
    ...
    if (!data.update) this.setState(data);
    else this.setState({ ...this.state, ...data });
  }
  ...
  change(e, v, i, ni) {
    ...
    this.setState((state) => {
      ...
      if (element.firstValue === undefined)
        element.firstValue =
          element.type === "select"
            ? v.options[Number(element.value)]
            : element.value;
      ...
      element.value =
        element.type === "checkbox"
          ? target.checked
          : element.type === "color"
            ? this.hexToRgb(target.value)
            : element.type === "select"
              ? v.options[Number(target.value)]
              : target.value;
      ...
      if (element.type === "color" && element.firstValue) {
        ...
        element.type === "select" &&
        element.firstIndexValue !== undefined
      ) {
        if (change === "new" || change === "same") {
    ...
    switch (v.type) {
      case "textarea": {
        return (
          <textarea
            required={v.required}
            onChange={(e) => this.change(e, v, i, ni)}
            placeholder={v.label}
            type="text"
      ...
      case "select": {
        return (
          <div className="flex gap-2 items-center flex-col justify-center">
            <label className="w-full text-center truncate" for={v.name}>
              {v.label}
            </label>
            <select
              required={v.required}
              ...
              className="w-full h-auto p-2 bg-green-900 rounded-lg"
              name={v.name}
              defaultValue={v.indexValue || v.value}
            >
              {v.options.map((nv2, ni2) => {
                return <option value={ni2}>{nv2.label}</option>;
              })}
            </select>
          </div>
        );
      ...
      case "input": {
        return (
          <input
            onChange={(e) => this.change(e, v, i, ni)}
            placeholder={v.label}
            className="bg-green-900 rounded-lg p-2 w-full"
            type="text"
          />
        );
      }
        ...
        return (
          <div className="flex gap-2 items-center w-full">
            <input
              ...
              max={v.max}
            />
            <label className="w-full break-words" for={v.name}>
              {v.label}
            </label>
          </div>
        );
      ...
      case "button": {
        return (
          <div className="flex gap-2 items-center">
            <button
              onClick={(e) => this.change(e, v, i, ni)}
              className="appearance-none bg-green-900 border-green-900 checked:bg-green-500 border-2 p-0.5 rounded-lg w-full"
              name={v.name}
            >
              {v.label}
            </button>
      ...
      case "color": {
        return (
          <div className="flex gap-2 items-center w-full">
            <input
              required={v.required}
              defaultValue={v.value && v.value.b && this.rgbToHex(v.value)}
              onChange={(e) => this.change(e, v, i, ni)}
        ...
        return (
          <div className="flex gap-2 items-center w-full">
            <input
              ...
              name={v.name}
            />
            <label className="w-full" for={v.name}>
              {v.label}
            </label>
          </div>
        );
      ...
      case "number": {
        return (
          <div className="flex gap-2 items-center">
            ...
            />
            <label className="text-ellipsis w-20" for={v.name}>
              {v.label}
            </label>
          </div>
        );
    ...
    return (
      <div className={this.sides[this.state.side]}>
        <div className="w-1/4 h-full p-2">
          <form
            id="menuside"
            className="w-full bg-black rounded-lg bg-opacity-90 h-full flex flex-col p-2 gap-4 justify-center select-none"
          >
            ...
            <div className={this.main[this.state.main]}>
              {this.state.elements.map((v, i) => {
                if (v.inline)
                  if (v.category)
                    return (
                      <div className="flex justify-center flex-col items-center break-all">
                        <input
                          defaultChecked={v.value || v.checked}
                          className="appearance-none peer bg-green-900 border-green-900 checked:bg-green-500 border-2 w-4 h-4 rounded"
                          type="checkbox"
                          id="category"
                          name="category"
                          value="category"
                        />
                        {v.img && (
                          <div className="w-full hidden peer-checked:flex justify-center items-center">
                            <img
            ...
            {this.state.messages && (
              <div className="w-full">
                {this.state.messages.map((v) => {
                  return (
                    <div className="w-full flex bg-blue-500 rounded p-2 flex-col break-words">
            ...
            {this.state.pages && (
              <div className="flex justify-between gap-4 font-bold">
                <button
                  type="xd"
                  onClick={(e) => this.goPage(e, -1)}
                  className="rounded-lg bg-green-400 w-min p-2"
                >
                  Poprzednia
                </button>
                ...
                <div className="flex gap-2 items-center">
                  <input
                    onChange={(e) => this.goPage(e)}
                    value={this.state.currentPage || 0}
                    className="appearance-none text-center bg-green-900 border-green-900 border-2 w-14 h-7 rounded out-of-range:text-red-500"
                    type="number"
                    name="page"
                    min={0}
                    defaultValue={this.state.currentPage || 0}
                ...
                <button
                  type="xd"
                  onClick={(e) => this.goPage(e, 1)}
                  className="rounded-lg text-white bg-green-400 w-min p-2"
                >
                  Następna
                </button>
              </div>
            ...
            <div className="flex justify-between gap-4 font-bold">
              ...
              {this.state.pages && (
                <div className="text-center flex gap-0.5">
                  <div>{this.state.currentPage}</div>
                  <div>/</div>
                  <div>{this.state.pages.length - 1}</div>
                </div>
              )}
              <button
                onClick={(e) => this.cancel(e)}
                className="rounded-lg text-red-500 bg-green-400 w-min p-2"
              >
                Anuluj
              </button>
            </div>
